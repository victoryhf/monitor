...
def getAlert():
    ...
    for i in ['T', 'A', 'B']:
        ...
        #total,当节点为tais时候，表示只取log日志list中第一个元素，因为求的是总和，如果有两条或以上该节点异常日志，则两条日志的该告警相同
        log.append(getNodeData('tais', i)[0])        #total
        log.append(getNodeData('mas', i)[0])         #total

        '''tais容量日志'''
        log.append(getNodeCapacity('tais', i)[0])    #total
...
def text2html():
	...
	if i[0]['type'] == 'Queue监控':
        ...
        elif (i[0]['type'] == '节点监控' or i[0]['type'] == '容量监控' ):
	...
	#增加容量监控告警信息显示(tais)
              ...
              if i[0]['name'] == 'tais':
                  alert_desc = "<td>tais_capacity_total:request:percentage=" + str(i[0]['capacity']) + ":" + str(i[0]['total_request']) + ":" + str(i[0]['percentage']) + "</td>"       
        elif i[0]['type'] == 'Sla监控':
	...
	#增加容量监控告警信息显示(tais)
            ...
            if i[1]['name'] == 'tais':
                alert_desc = "<td>tais_capacity_total:request:percentage=" + str(i[1]['capacity']) + ":" + str(i[1]['total_request']) + ":" + str(i[1]['percentage']) + "</td>"
