...
#my $url_domain = "https://172.21.195.117:13089";
my $url_domain = "https://splunk-inner.99bill.com:13089";
...
my $search_hash = {
        ...
        "aps" => 'search source="/opt/log/app-aps/app-aps.log" authNetId=01021100 "SendReq" OR "RecvRsp" |eval result=if(position=="SendReq",1,0) | stats last(date_hour) as hour, last(date_minute) as minute, count(eval(result=1)) as request,count(eval(result=0)) as response by host|eval time = hour +":" + minute|fields host, time, request, response',
